	...
	*/
	_processResults(data) {
		...
		for (let i = data.results.length - 1; i >= 0; i--) {
			...
			if (data.results[i].bounds) results[i].bounds = L.latLngBounds([data.results[i].bounds.southwest.lat, data.results[i].bounds.southwest.lng], [data.results[i].bounds.northeast.lat, data.results[i].bounds.northeast.lng]);
...
*/
var OpenCageGeocodingControl = class extends L.Control {
	constructor(options = {}) {
		super(options);
		this.options = {
	...
	*/
	_geocodeResult(results) {
		...
		} else L.DomUtil.addClass(this._errorElement, "leaflet-control-opencage-geocoding-error");
	}
	/**
	...
	*/
	_keydown(e) {
		const select = (dir) => {
			if (this._selection) {
				L.DomUtil.removeClass(this._selection.firstChild, "leaflet-control-opencage-geocoding-selected");
			...
			if (this._selection) L.DomUtil.addClass(this._selection.firstChild, "leaflet-control-opencage-geocoding-selected");
		};
		switch (e.keyCode) {
			case 38:
				select(-1);
...
//#endregion
export { OpenCageGeocoder, OpenCageGeocodingControl, OpenCageGeocodingControl as default, openCageGeocoding };

//# sourceMappingURL=L.Control.OpenCageGeocoding.esm.js.map